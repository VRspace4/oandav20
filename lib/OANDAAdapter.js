...
// getPriceStream
OandaAdapter.prototype.getPriceStream = function (accountId, instrument, callback) {

  this._sendStreamRequest({
    method: "GET",
    path: "/v3/accounts/" + accountId +"/pricing/stream?instruments=" + instrument
  }, function (error, body, statusCode) {
    if (error) {
      if (body && body.message) {
        console.error("[ERROR] Response from Oanda", statusCode + " Error: " + body.message + " (OANDA error code " + body.code + ")");
 ...
 * @param context
 */
OandaAdapter.prototype.subscribeTransactions = function (accountId, listener, context) {

  var existingSubscriptions = this.getHandlers("transactions/");

  // Transactions stream needs an accountId to be passed for streaming transactions, though transactions for a connection are same anyway
  if (!this.streamTransactions) {
    this.streamTransactions = _.throttle(this._streamTransactions.bind(this, accountId));
  }

  this.off("transactions/", listener, context);
  this.on("transactions/", listener, context);
...
OandaAdapter.prototype.unsubscribeTransactions = function (symbol, listener, context) {
  this.off("transactions/", listener, context);
  this.streamTransactions();
};
...
OandaAdapter.prototype._streamTransactions = function (accountId) {

  var changed;

  this.transactionsSubscriptions = Object.keys(this.getHandlers()).reduce(function (memo, event) {
    var match = event.match("^transactions/(.+)$");
    if (match) {
      memo.push(match[1]);
    }
    return memo;
  ...
  changed = !this.lastTransactionsSubscriptions || this.lastTransactionsSubscriptions !== this.lastTransactionsSubscriptions;

  this.lastTransactionsSubscriptions = this.transactionsSubscriptions;
  ...
  if (this.transactionsRequest) {
    this.transactionsRequest.abort();
  }

  // if (this.transactionsSubscriptions === "") {
  //   return;
  // }
  ...
  this.transactionsRequest = httpClient.sendRequest({
      hostname: this.streamHost,
      method: "GET",
      path: "/v3/accounts/" + accountId + "/transactions/stream",
      headers: {
    ...
    this._onTransactionsResponse.bind(this, accountId),
    this._onTransactionsData.bind(this)
  );
};

OandaAdapter.prototype._onTransactionsResponse = function (accountId, error, body, statusCode) {
  if (statusCode !== 200) {
    if (body && body.disconnect) {
      this.trigger("message", accountId, "Transactions streaming API disconnected.\nOanda code " + body.disconnect.code + ": " + body.disconnect.message);
    } else {
      this.trigger("message", accountId, "Transactions streaming API disconnected with status " + statusCode);
    }
  }
  clearTimeout(this.transactionsTimeout);
  this.transactionsTimeout = setTimeout(this._transactionsHeartbeatTimeout.bind(this), 10000);
...
OandaAdapter.prototype._onTransactionsData = function (data) {
  ...
  data.split(/\r\n/).forEach(function (line) {
    ...
    if (line) {
      ...
      try {
        ...
        console.error("[ERROR] Unable to parse Oanda transactions subscription update", this._transactionsBuffer.join("\n"), error);
        this._transactionsBuffer = [];
        return;
      }
      this._transactionsBuffer = [];
      ...
      if (update.type === "HEARTBEAT") {
        ...
        this.trigger("transactions/", update);
      }
    }
  }, this);
};

OandaAdapter.prototype._transactionsHeartbeatTimeout = function () {
  console.warn("[WARN] OandaAdapter: No heartbeat received from transactions stream for 10 seconds. Reconnecting.");
  delete this.lastTransactionsSubscriptions;
  this.streamTransactions();
};
...
OandaAdapter.prototype._streamPrices = function (accountId) {
    ...
    this.pricesRequest = httpClient.sendRequest({
            hostname: this.streamHost,
            method: "GET",
            path: "/v3/accounts/101-001-1712748-001/pricing/stream?&instruments=AUD_USD",
            headers: {
                Authorization: "Bearer " + this.accessToken,
                Connection: "Keep-Alive"
            },
    ...
    data.split(/\r\n/).forEach(function (line) {
        ...
        if (line) {
            ...
            if (update.type === "HEARTBEAT") {
                ...
                update.time = new Date(update.time);
                this.trigger("price/" + update.instrument, update);
            }
        }
    }, this);
};
...
OandaAdapter.prototype._sendStreamRequest = function (request, callback) {

  request.hostname = this.streamHost;
  request.headers = request.headers || {
    Authorization: "Bearer " + this.accessToken
  };
  request.secure = this.secure;
